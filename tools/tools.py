...
def binning(
    ...
    raws.fillna(
        1e-20, inplace=True
    )  # TODO: Is it reasonable?: Fill Nan value as small value for bheight, TranPeak, flight, pwrline, dtm, channel data.
...
def binning_normal(
    ...
    for i_line, line in enumerate(line_no[istart:iend]):
        ...
        for tar_bin in bins:
            ...
            if len(sub_df) > 1:
                mean_std_features = (std_features).mean(axis=0)
                var = ((std_features - mean_std_features) ** 2).mean(axis=0)
                std = np.sqrt(var)
            else:
                std = np.zeros(std_features.shape[1])
