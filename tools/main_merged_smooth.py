...
def main(
    ...
    # reference model
    reference_model = np.ones(nP) * np.log(1.0 / rho_ref)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--smallness", type=float, help="Smallness", required=True
    )
    parser.add_argument(
        "-r",
        "--reference",
        type=float,
        help="Reference model resistivity",
        required=True,
    )
    # 3. Parse arguments
    ...
    main(
        ...
        ncpu=args.ncpu,
        alpha_s=args.smallness,
        rho_ref=args.reference,
    )
